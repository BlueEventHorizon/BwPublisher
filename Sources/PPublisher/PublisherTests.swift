...
class PublisherTests: XCTestCase {
    ...
    func testBwTaskBasic() {

        let expectation1 = XCTestExpectation(description: "expectation1")

        let task0 = BwTaskStart.firstTask { (task) in
            print("*** 0")
            usleep(500000)
            task.signal(())
        }

        let task1: BwTask<Int> = task0.wait { (task, result) in
            print("*** 1.result = \(result)")
            usleep(500000)
            task.signal(1)
        }
        let task2: BwTask<String> = task1.wait { (task, result) in
            print("*** 2.result = \(result)")
            usleep(500000)
            task.signal("*")
        }
        let _: BwTask<Bool> = task2.wait { (task, result) in
